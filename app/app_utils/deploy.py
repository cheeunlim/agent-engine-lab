...
from vertexai._genai.types import AgentEngine, AgentEngineConfig, IdentityType, common
...
@click.option(
    ...
    type=int,
    default=8,
    help="Container concurrency (default: 8)",
)
@click.option(
    "--num-workers",
    type=int,
...
def deploy_agent_engine_app(
    ...
    # Lazy binding of customization
    #Added following config for db based session and tracing
    if db_url != None:#To connect DB using private network, use following link to setup database
    ...
    agent_instance.session_service_builder = session_service
    agent_instance.memory_service_builder = CustomMemoryBankService(project, location)
    ...
    config.context_spec = common.ReasoningEngineContextSpec(
        memory_bank_config=common.ReasoningEngineContextSpecMemoryBankConfig(
            similarity_search_config=common.ReasoningEngineContextSpecMemoryBankConfigSimilaritySearchConfig(
            ...
            generation_config=common.ReasoningEngineContextSpecMemoryBankConfigGenerationConfig(
                model=f"projects/{project}/locations/{model_location}/publishers/google/models/gemini-3.1-flash-lite-preview",
            ),
            customization_configs=[
                common.MemoryBankCustomizationConfig(
                    enable_third_person_memories=True,
                    consolidation_config=common.MemoryBankCustomizationConfigConsolidationConfig(
                        revisions_per_candidate_count=1
                    ),
                    memory_topics=[
                        common.MemoryBankCustomizationConfigMemoryTopic(
                            managed_memory_topic=common.MemoryBankCustomizationConfigMemoryTopicManagedMemoryTopic(
                                managed_topic_enum=common.ManagedTopicEnum.USER_PERSONAL_INFO
                            )
                        ),
                        common.MemoryBankCustomizationConfigMemoryTopic(
                            managed_memory_topic=common.MemoryBankCustomizationConfigMemoryTopicManagedMemoryTopic(
                                managed_topic_enum=common.ManagedTopicEnum.USER_PREFERENCES
                            )
                        ),
                        common.MemoryBankCustomizationConfigMemoryTopic(
                            managed_memory_topic=common.MemoryBankCustomizationConfigMemoryTopicManagedMemoryTopic(
                                managed_topic_enum=common.ManagedTopicEnum.KEY_CONVERSATION_DETAILS
                            )
                        ),
                        common.MemoryBankCustomizationConfigMemoryTopic(
                            managed_memory_topic=common.MemoryBankCustomizationConfigMemoryTopicManagedMemoryTopic(
                                managed_topic_enum=common.ManagedTopicEnum.EXPLICIT_INSTRUCTIONS
                            )
                        ),
                        common.MemoryBankCustomizationConfigMemoryTopic(
                            custom_memory_topic=common.MemoryBankCustomizationConfigMemoryTopicCustomMemoryTopic(
                    ...
                    #    common.MemoryBankCustomizationConfigGenerateMemoriesExample()
                    #]
                )
            ]
        )
